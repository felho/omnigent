...
class RunnerEventDispatcher:
    ...
    def negotiated(self) -> bool:
        """Whether a server has advertised event ingestion in this process."""
        return self._negotiated

    async def wait_for_first_ready(self) -> bool:
        """Let mixed-version servers fall back to HTTP, without waiting on an outage."""
        if self._negotiated:
            return True
        try:
    ...
    def ready(self, send: Callable[[str], Awaitable[None]]) -> None:
        """Enable delivery only after the new tunnel has received event.ready."""
        self._send = send
        ...
        while True:
            ...
            try:
                ...
                async with self._locks.setdefault(session_id, asyncio.Lock()):
                    ack = await self._deliver(session_id, events)
                if not result.done():
...
class TunnelEventClient(httpx.AsyncClient):
    ...
    async def post(self, *args: Any, **kwargs: Any) -> httpx.Response:
        ...
        # acknowledgement; this tunnel protocol currently returns only a
        # prefix count, so keep their existing HTTP batch path.
        if (
            (len(events) > 1 and events[0]["type"] == "external_conversation_item")
        ...
        if ack.applied == len(events):
            return httpx.Response(202, json={"queued": False}, request=request)
