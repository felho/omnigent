...
async def test_serve_tunnel_caps_backoff_low_until_first_connection(
    ...
    """Local initial reconnects stay within the boot-phase cap."""
    attempts = {"count": 0}
    sleeps: list[float] = []

    async def _serve_once(*_args: Any, **_kwargs: Any) -> None:
        """Refuse connections until the test has seen enough retries."""
        attempts["count"] += 1
        if attempts["count"] <= 8:
    ...
    # The cap is an explicit boot-latency contract.
...
async def test_serve_tunnel_keeps_full_backoff_cap_for_remote_initial_connect(
    ...
    """A remote initial connect retains the full reconnect cap."""
    attempts = {"count": 0}
    sleeps: list[float] = []
...
async def test_serve_tunnel_releases_boot_backoff_cap_after_first_connection(
    ...
    # The post-connect outage may back off past the boot cap.
    assert max(sleeps) > serve_module._MAX_INITIAL_CONNECT_DELAY_S, (
